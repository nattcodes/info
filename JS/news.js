...
fetch("news.json") //fetch api from the local json file
    ...
    .then((data) => {
        let newsTitle = "";
        data.splice(11, 1).map((newsItem) => { //splice, get a particular section of the files and then map them
...
post.addEventListener("click", function(e) {
        ...
        pTag.appendChild(commentText); //link the value of the textarea to the p tag element
        document.querySelector(".box").appendChild(h3Tag); //connect each value gotten from the h3 tag to the mother container ".box"
